...
makerBot.hears('🛠 Create Bot', async (ctx) => {
  ...
  try {
    ...
    ctx.reply('Send your bot token from @BotFather to create your bot:', backKeyboard);
  } catch (error) {
...
makerBot.hears('🗑️ Delete Bot', async (ctx) => {
  ...
  try {
    ...
    ctx.reply('Send the bot token you want to delete:', backKeyboard);
  } catch (error) {
...
makerBot.on('text', async (ctx) => {
  ...
  try {
    ...
    if (user.step === 'create_bot') {
      ...
      ctx.reply(`✅ Your bot @${botInfo.username} created successfully! Send /panel to manage it.`, mainMenu);
      await User.findOneAndUpdate({ userId }, { step: 'none' });
